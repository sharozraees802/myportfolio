import React from 'react'

const R = props => {
  return (
    <svg {...props} version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000">
    <title>R</title>
    {/* <defs>
      <image  width="1000" height="1000" id="img1" href="data:image/png;base64,"/>
    </defs> */}
    {/* <style>
      .s0 { fill: #000000;stroke: #000000 } 
    </style> */}
    <use id="Background" href="#img1" x="0" y="0"/>
    <g id="Folder 1">
      <path id="Shape 5" fillRule="evenodd" className="s0" d="m570 429l84 40-8 10c0 0-60 8-61 8 74.5 54 213 78 212 78 21 36.7 54 61 54 61l4 17c0 0-16.5 2-17 1-24.5-46.7-269-141-269-141l-12 16-13-37 6-7-6-37c7-16.5 26-9 26-9zm-5.5 26q1.4 1 3 2 0-0.3 0-0.5zm5.5 15.2c0 0.3-0.4 9 0.1 9.1 9.3 1.4 20.4-4.8 29.9-5.3 0.8-0.1 7.8-1.8 7.4-2.6-1.3-2.9-11.2-4.8-13.7-5.6-8.3-2.6-18.4-4.3-26.2-8.8 0.2 4.5 2.5 8.6 2.5 13.2z"/>
    </g>
  </svg>
  )
}

R.propTypes = {}

export default R
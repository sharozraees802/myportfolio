import React from 'react'

const O= props => {
  return (
    <svg {...props} version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000">
    <title>O</title>
    {/* <defs>
      <image  width="1000" height="1000" id="img1" href="data:image/png;base64,"/>
    </defs> */}
    {/* <style>
      .s0 { fill: #000000;stroke: #000000 } 
    </style> */}
    <use id="Background" href="#img1" x="0" y="0"/>
    <g id="Folder 1">
      <path id="Layer 2" fillRule="evenodd" className="s0" d="m704 425c0 3 31.5 41.5 31 43 13 8.5 2 18 2 18 0 0-26 38.5-28 38-12.5 14-19-1-19-1 0 0-17-21.5-24-34-7-12.5-2-20-2-20l21-48c7.5-13.5 19 4 19 4zm-7 29c0 0-17.5 25-16 27 1.5 2 26 23 26 23l11-24z"/>
    </g>
  </svg>
  )
}

O.propTypes = {}

export default O
import React from 'react'

const E = props => {
  return (
    <svg {...props} version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000">
    <title>E</title>
    {/* <defs>
      <image  width="1000" height="1000" id="img1" href="data:image/png;base64,"/>
    </defs> */}
    {/* <style>
      .s0 { fill: #000000;stroke: #000000 } 
    </style> */}
    <use id="Background" href="#img1" x="0" y="0"/>
    <g id="Folder 1">
      <path id="Shape 4" className="s0" d="m523 465c-5.2 0-15-2-15-2l-2 28 24-5 10 8-12 6-21 2-3 16c0 0 24.8-9.2 25-9 18-7.8 7.4 10.6 7 10-11.4 24.4-39.6 23.2-40 23-14 3-10-16.2-10-16 3.6-37.6 3-84 3-84 3-10 16-6 16-6l21 11c17.4 9.2 2.2 18-3 18z"/>
    </g>
  </svg>
  )
}

E.propTypes = {}

export default E